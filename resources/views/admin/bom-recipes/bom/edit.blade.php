...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="fs-22 mb-0">Edit BOM</h2>
    ...
    <!-- Container -->
    <form method="POST" action="{{ route('inventory.bom.update', $bom->bom_code) }}"
                    ...
                    <div class="row g-3">
                            ...
                            <select class="form-select single-select @error('product_id') is-invalid @enderror" 
                                ...
                                @foreach($products as $product)
                                    <option value="{{ $product->product_id }}" 
                                            {{ old('product_id', $bom->product_id) == $product->product_id ? 'selected' : '' }}>
                        ...
                        <div class="col-12">
                            ...
                            <input type="text" 
                                   ...
                                   id="bom_version"
                                   name="bom_version" 
                                   value="{{ old('bom_version', $bom->bom_version) }}"
                        ...
                        <div class="col-12">
                            ...
                            @php
                                $inputDob = old('effective_date', $bom->effective_date);
                            @endphp
                        ...
                        <div class="col-12">
                            ...
                            <textarea class="form-control @error('notes') is-invalid @enderror"
                                    ...
                                    name="notes" 
                                    rows="3"
                                    placeholder="Additional notes...">{{ old('notes', $bom->notes) }}</textarea>
        ...
        <div class="col-lg-8">
            ...
            <div class="card custom">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <a href="{{ route('inventory.bom.index') }}" class="btn btn-outline-secondary">
...
@section('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const existingItems = @json($bomItems);
            existingItems.forEach(item => {
                addBOMItem();
                const lastItem = document.querySelector('.bom-item:last-child');
                const materialSelect = lastItem.querySelector('.material-select');
                    $(materialSelect).val(item.material_id).trigger('change.select2');
                const itemTypeSelect = lastItem.querySelector('select[name*="[item_type]"]');
                    $(itemTypeSelect).val(item.item_type).trigger('change.select2');
                lastItem
                    .querySelector('.quantity-input')
                    .value = item.quantity_required;
                const uomSelect = lastItem.querySelector('select[name*="[uom_id]"]');
                    $(uomSelect).val(item.uom_id).trigger('change.select2');
                lastItem
                    .querySelector('input[name*="[scrap_percentage]"]')
                    .value = item.scrap_percentage || 0;
                updateMaterialInfo(lastItem.querySelector('.material-select'));
            });
        });
    </script>
